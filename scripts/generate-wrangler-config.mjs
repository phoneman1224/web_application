...
function discoverD1Database(name) {
  try {
    ...
    if (db) {
      console.log(`   ✅ Found D1 database "${name}": ${db.uuid}`);
      return db.uuid;
    } else {
      console.log(`   ⚠️  D1 database "${name}" not found (will need to be created)`);
      return null;
    }
  } catch (error) {
    console.log(`   ⚠️  Could not list D1 databases (may not have wrangler configured)`);
    return null;
  }
}
